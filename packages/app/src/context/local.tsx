  ...
  init: () => {
    ...
    const agent = (() => {
      ...
      return {
        ...
        set(name: string | undefined) {
          setStore("current", name ?? list()[0]?.name ?? "code")
        },
        move(direction: 1 | -1) {
    ...
    const model = (() => {
      ...
      const fallbackModel = createMemo(() => {
        ...
        // Return undefined if no default model found - this allows the app to start
        // and prompt the user to connect a provider
        return undefined
      })
      ...
      return {
        ...
        set(model: ModelKey | undefined, options?: { recent?: boolean }) {
          batch(() => {
            ...
            if (effectiveModel) {
              setEphemeral("model", agent.current().name, effectiveModel)
            }
    ...
    const file = (() => {
      ...
      return {
        ...
        init,
        expand(path: string) {
          // Handle root path specially - just load without setting node properties
          if (path === "") {
            list("")
            return
          }
