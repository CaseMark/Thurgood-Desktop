...
export const { use: useLayout, provider: LayoutProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    return {
      ...
      fileBrowser: {
        opened: createMemo(() => store.fileBrowser?.opened ?? false),
        open() {
          setStore("fileBrowser", "opened", true)
        },
        close() {
          setStore("fileBrowser", "opened", false)
        },
        toggle() {
          setStore("fileBrowser", "opened", (x) => !x)
        },
        width: createMemo(() => store.fileBrowser?.width ?? 280),
        resize(width: number) {
          setStore("fileBrowser", "width", width)
        },
      },
      session: {
