...
fn get_sidecar_port() -> u32 {
    option_env!("THURGOOD_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("THURGOOD_PORT").ok())
...
fn spawn_sidecar(app: &AppHandle, port: u32) -> CommandChild {
    ...
    let (mut rx, child) = app
        .shell()
        .sidecar("thurgood-cli")
        .unwrap()
        .env("THURGOOD_EXPERIMENTAL_ICON_DISCOVERY", "true")
        .env("THURGOOD_CLIENT", "desktop")
        .env("XDG_STATE_HOME", &state_dir)
        .args(["serve", &format!("--port={port}")])
        .spawn()
        .expect("Failed to spawn thurgood");
                ...
                let child = if should_spawn_sidecar {
                    ...
                    loop {
                        if timestamp.elapsed() > Duration::from_secs(7) {
                            let res = app.dialog()
                              .message("Failed to spawn Thurgood Server. Copy logs using the button below and send them to the team for assistance.")
                ...
                let mut window_builder =
                    WebviewWindow::builder(&app, "main", WebviewUrl::App("/".into()))
                        ...
                        .initialization_script(format!(
                            r#"
                          window.__THURGOOD__ ??= {{}};
                          window.__THURGOOD__.updaterEnabled = {updater_enabled};
                          window.__THURGOOD__.port = {port};
                        "#
                        ));
